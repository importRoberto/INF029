...
//Ponteiro que aponta para as estruturas auxiliares
estrutura *vetorPrincipal;
...
void salvarDados() {
    ...
    for (int i = 0; i < TAM; i++) {
        estrutura *aux = &vetorPrincipal[i];
        if (aux->vet != NULL) {
            int posicao = i + 1;
            fwrite(&posicao, sizeof(int), 1, arquivo);
            fwrite(&aux->tamanho, sizeof(int), 1, arquivo);
            fwrite(&aux->posAtual, sizeof(int), 1, arquivo);
            fwrite(aux->vet, sizeof(int), aux->posAtual, arquivo);
        }
    }
    fclose(arquivo);
}
...
void carregarDados() {
    ...
    int posicao;
    while (fread(&posicao, sizeof(int), 1, arquivo) == 1) {
        int tamanho, posAtual;
        fread(&tamanho, sizeof(int), 1, arquivo);
        fread(&posAtual, sizeof(int), 1, arquivo);

        criarEstruturaAuxiliar(posicao, tamanho);
        estrutura *aux = &vetorPrincipal[posicao - 1];
        aux->posAtual = posAtual;

        fread(aux->vet, sizeof(int), posAtual, arquivo);
    }
    fclose(arquivo);
}
...
void inicializar()
{
    // Aloca o espaço de memória para o vetor principal
    vetorPrincipal = malloc(sizeof(estrutura) * TAM);

    // Inicializa todos os ponteiros de estruturas auxiliares
    for(int i = 0; i < TAM; i++){
...
int partition(int arr[], int low, int high) {
    // Escolhe o último elemento como referência
    int pivot = arr[high];
    
    // Índice do menor elemento
    int i = (low - 1);

    // Compara cada elemento com a referência
    ...
    // Troca a referência com o elemento do índice do menor
    swap(&arr[i + 1], &arr[high]); 
    return (i + 1); // Retorna o índice da referência
}
